...
#     time_since_played = time_since_played * (last_played != -1)
...
def do_update(mus, rds, sum_1, sum_2, last_played, idx_map, idx_map_back, cur_time_step, num_competitors_seen_this_time_step, max_rd, c2):
    ...
    new_mus, new_rd2s = glicko_update(active_mus, active_rd2s, sum_1, sum_2)
    new_rds = jnp.sqrt(new_rd2s)

    # new_rds = time_update(new_rd2s, active_last_played, cur_time_step, max_rd, c2)
...
def batched_glicko_update(idx, prev_val, max_rd, c2, q, q2, three_q2_over_pi2):
    ...
    og_last_played = last_played.copy()
    ...
    cur_mus = mus[comp_idxs]
    cur_rds = rds[comp_idxs]
    cur_last_played = og_last_played[comp_idxs]
    time_since_played = cur_time_step - cur_last_played

    time_since_played = time_since_played * (cur_last_played != -1)
    cur_rds_squared = jnp.minimum(jnp.square(cur_rds) + (time_since_played * c2), max_rd ** 2.0)
    ...
    rds = rds.at[idx_1].set(jnp.sqrt(cur_rds_squared[0]))
    rds = rds.at[idx_2].set(jnp.sqrt(cur_rds_squared[1]))
...
def main():
    ...
    matchups = jnp.vstack([matchups, jnp.array([[dataset.num_competitors, dataset.num_competitors]])])
    outcomes = jnp.concat([outcomes, jnp.array([0.0], dtype=DTYPE)])
    time_steps = jnp.concat([time_steps, jnp.array([time_steps[-1]]) + 1])
